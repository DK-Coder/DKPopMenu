...
- (void)addKeyframeAnimationWithDuration:(NSTimeInterval)duration path:(CGPathRef)path animationKey:(NSString *)key complete:(void (^)())block
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)((duration - 0.f) * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (block) {
            block();
        }
    });
...
- (void)addScaleAnimationWithDuration:(NSTimeInterval)duration from:(CGFloat)fromValue to:(CGFloat)toValue animationKey:(NSString *)key complete:(void (^)())block
{
    ...
    [self addAnimation:scaleAnimation forKey:key];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)((duration - 0.f) * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (block) {
            block();
        }
